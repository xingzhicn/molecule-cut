...
molecule and every permitted complete execution of their six-step cutting algorithm, we
...
families attain the bound for every \(|M_U|\ge4\), so the benchmark constant is
exact.  As a scoped consequence, the intrinsic score satisfies
...
\begin{theorem}[Sharp Toy I benchmark bound]\label{thm:sharp}
...
Together with Theorem~\ref{thm:sharp}, these families prove
\begin{equation}
...
Theorem~\ref{thm:sharp} gives \(\good\ge\lceil(n-1)/3\rceil\) for it.  Since \(v_d\) is
...
\end{verbatim}
On the release snapshot, these checks report 653 passing tests and a clean
Ruff check.  The finite counts above are regression evidence only; they are
not used in the all-size proof.
