% E-JC submission format. The journal's official style file is unchanged.
...
\MSC{05C05, 05C85, 05C90}
\specs{SUBMITTED}{TBD}{TBD}{00000}
\Copyright{The author.}

\title{The Sharp Benchmark Constant for the Deng--Hani--Ma Two-Layer Cutting Algorithm}
\author{Zhangjinba}
\authortext{}{Independent Researcher.}

\begin{document}
\maketitle

\begin{abstract}
We consider the two-layer Toy I molecule model arising in the combinatorial
part of the Deng--Hani--Ma hard-sphere argument.  For every finite Toy I
molecule and every permitted execution of their six-step cutting algorithm, we
prove that the number of good two-atom components is at least
\(\lceil(|M_U|-1)/3\rceil\), improving the source benchmark \((|M_U|-1)/5\).
The proof is an amortised flow argument: orphaned cross-bonds are paid for by
persistent fixed-end tokens created by earlier cuts.  Three explicit path
families attain the bound under stated permitted executions for every
\(|M_U|\ge4\), so the benchmark constant is exact.  As a scoped consequence,
the intrinsic score satisfies
\(v_d(M)\ge\lceil\rho(M)/3\rceil-10d\) within Toy I.  No claim is made for
larger toy classes, arbitrary legal cuts, or the global molecule problem.
\end{abstract}
...
execution}.  Its output is elementary after the prescribed three-atom
\(\{343\}\) cleanup macro is resolved into one \(\{3\}\) and one \(\{33\}\);
the macro contributes one to \(\good\) and none to \(\bad\).  Separately, a
\emph{legal complete cut} means any sequence of
...
The first identity in \eqref{eq:counting} below asserts that every \(B\) event
contributes exactly one \(\{33\}\), which requires the selected upper atom to
have degree three.  The source asserts the conclusion --- in the proof of
\cite[Proposition 11.7]{DHM}, each cut upper atom ``either forms a component by
itself or belongs to a \(\{33\}\) molecule'' --- while its step (2) also carries
the defensive clause ``cut \(m\) as free from \(\{n,m\}\) if \(n\) has deg
\(4\)''.  The two are consistent only because that clause is vacuous on Toy I.
We prove this first.

\begin{lemma}[Upper-layer degree at operation (b)]\label{lem:degree}
In a Definition-11.4 execution on a finite Toy I molecule, let \(n\in\MU\) be
the atom selected at step (1) and suppose its cross-bond partner \(m\in\MD\)
has not yet been cut.  Then \(n\) has degree three, unless \(n\) is the first
atom selected by the run, in which case \(m\) has degree four.  Consequently
every \(B\) event cuts a genuine \(\{33\}\), and the degree-four clause of step
(2) is never invoked.
\end{lemma}

\begin{proof}
In Toy I every upper atom carries exactly one cross-bond, which occupies one of
its at most two child slots; hence every upper atom has at most one child in
\(\MU\).  Summing the number of \(\MU\)-children over \(\MU\) counts the
\(|\MU|-1\) intralayer bonds, so with every term at most one, exactly one atom
\(n_0\in\MU\) has no \(\MU\)-child.

No top fixed end ever appears in \(\MU\): step (1) selects only atoms whose
\(\MU\)-children have all been cut, so cutting an upper atom creates no top
fixed end above it, and cuts inside \(\MD\) create none either.  Hence the fixed
ends at the selected \(n\) are exactly the bottom fixed ends created by cutting
children of \(n\).  By hypothesis \(m\) survives, so the cross-bond contributes
none, and the remaining children of \(n\) lie in \(\MU\) and have all been cut.
Therefore \(\deg(n)=4-\#\{\MU\text{-children of }n\}\), which is \(3\) if
\(n\neq n_0\) and \(4\) if \(n=n_0\).
...
moment \(\MD\) is still a tree with no fixed ends, so every atom of \(\MD\) has
degree four.  Hence \(m\) has degree four and step (2) takes its ``otherwise''
branch, an \(A\) event.  Every selection with a degree-three partner therefore
has \(n\neq n_0\) and \(\deg(n)=3\).
\end{proof}
...
split, and is well defined by Lemma~\ref{lem:degree}; the second is the change
of the source potential; the third partitions the upper-layer atoms.
...
outer atoms have degree three, while the centre has degree four and no fixed
end, so its two parent slots give \(c_v+p_v\le2\).  The two internal bonds
contribute two parent slots.  Summing gives
...
\end{verbatim}
On August 21, 2026 these checks reported 675 passing tests, a clean Ruff check,
...
not prove a global positive lower bound for \(c_d^*\), an exact formula,
complexity classification, or optimal-cut theorem for \(W\), a result for Toy I
plus/II/III or the full molecule, a score bound for every
...
arXiv preprint arXiv:2408.07818v3 (2025), especially Section 11.2 and
Proposition 11.7.  \arxiv{2408.07818}.
...
(after Deng, Hani, Ma)}, S\'eminaire Bourbaki 1247 (2026).
\arxiv{2602.04407}.

\bibitem{BC}
Y.~Bruned and V.~Clarisse,
\emph{Kruskal-style algorithm for cubic Schr\"odinger equation molecule
reduction}, arXiv preprint arXiv:2603.23298 (2026).
\arxiv{2603.23298}.
